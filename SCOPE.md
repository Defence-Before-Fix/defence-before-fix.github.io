...
1. **Observed, not imagined.** The proposal names a case where the absence of this obligation
   ...
   see it happened. A hypothetical, however plausible, does not qualify. The method was formalised
   after real failures in cold readings, and its obligations are added the same way.
...
4. **Verifiable from the record.** An owner can tell from the project record alone, without
   asking the practitioner, whether the obligation was met. A MUST nobody can check is advice,
   and advice does not go in a normative document. The proposer says what an owner would look
   at.

5. **Costed.** The proposal states what the obligation costs the practitioner to meet and what
   it buys the owner, in the shape section [2](SPEC.md#2-when-the-method-applies) of SPEC.md already uses for the ways out of the
   method. An obligation with a cost and no buyer is declined; one with a buyer and no cost is
   suspicious and gets a second look.
