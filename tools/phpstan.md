...
**Language**: PHP · **Kind**: tool · **Readiness**: 🟢 · **Detector conformance**: 🟡 · **Checked**: 2026-09-08, version 2.2.13
...
| Detector | 7.1    | Yes          | `@phpstan-ignore` is a node comment a project rule can detect; the baseline is active only when included, [baseline](https://phpstan.org/user-guide/baseline)     |
| Detector | 7.2    | Partial      | `reportIgnoresWithoutComments` covers inline only; baseline and `ignoreErrors` carry no reason, [ignoring errors](https://phpstan.org/user-guide/ignoring-errors) |
